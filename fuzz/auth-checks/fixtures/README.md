# fuzz/auth-checks/fixtures

Regression fixtures for the auth-checks fuzz target. Each file here is a byte-for-byte crasher input that
once made `fuzz/auth-checks/target.mjs` throw something other than its documented error class, hang, or
