// RFC 6350 §3.2–§3.3 content lines — the RFC 5545 §3.1 grammar plus `group.` name prefixes: CRLF
// line breaks, folding at 75 octets (never inside a UTF-8 sequence), parameters with quoted values
// and RFC 6868 caret escapes, and TEXT escaping (RFC 6350 §3.4). A copy of @postroom/ical's lexer
// with vCard's error classes, so neither package depends on the other.
import { VCardError, VCardLimitError, VCardParseError } from './errors.js';
...
/** Octets a line may occupy before it must be folded, excluding the CRLF (RFC 6350 §3.2). */
...
export function decodeInput(input: string | Uint8Array, maxBytes: number): string {
  ...
  if (typeof input === 'string') {
    if (input.length > maxBytes || utf8Length(input) > maxBytes) {
      throw new VCardLimitError(`input exceeds ${String(maxBytes)} bytes`);
    }
    text = input;
  } else {
    if (input.byteLength > maxBytes) throw new VCardLimitError(`input exceeds ${String(maxBytes)} bytes`);
...
export function unfold(text: string, maxLines = Number.POSITIVE_INFINITY): LogicalLine[] {
  ...
  for (let i = 0; i < physical.length; i++) {
    ...
    if (out.length > maxLines) throw new VCardLimitError(`more than ${String(maxLines)} content lines`);
    if (first === 0x20 || first === 0x09) {
      // A continuation with nothing to continue: RFC 6350 has no meaning for it.
      if (p.trim() === '') {
        current = null;
        continue;
      }
      throw new VCardParseError('folded continuation line with no line before it', i + 1);
    }
...
export function parseContentLine(text: string, lineNo: number, options: ContentLineOptions = {}): ParsedContentLine {
  ...
  if (options.groups === true) {
    ...
    if (dot >= 0) {
      ...
      if (group === '' || group.includes('.')) throw new VCardParseError(`malformed group in "${clip(text)}"`, lineNo);
    }
  }
  if (name === '') throw new VCardParseError(`content line has no name: "${clip(text)}"`, lineNo);
  ...
  while (text[i] === ';') {
    ...
    if (pname === '') throw new VCardParseError(`empty parameter name in "${clip(text)}"`, lineNo);
    if (text[i] !== '=') {
      if (options.bareParamsAreTypes === true && (text[i] === ';' || text[i] === ':')) {
        addParam(params, 'TYPE', [text.slice(pStart, i)]);
        continue;
      }
      throw new VCardParseError(`parameter ${pname} has no "="`, lineNo);
    }
    i++;
    const values: string[] = [];
    for (;;) {
      if (text[i] === '"') {
        ...
        if (close < 0) throw new VCardParseError(`unterminated quoted value for ${pname}`, lineNo);
    ...
    if (text[i] !== ';' && text[i] !== ':') throw new VCardParseError(`junk after the value of ${pname}`, lineNo);
    addParam(params, pname, values);
  }
  if (text[i] !== ':') throw new VCardParseError(`content line has no ":" before its value: "${clip(text)}"`, lineNo);
...
export function formatContentLine(line: ContentLine, group: string | null = null): string {
  if (/[\r\n]/.test(line.value)) {
    throw new VCardError(`value of ${line.name} contains a raw line break; escape it as TEXT first`);
  }
...
/** TEXT escaping (RFC 6350 §3.4): backslash, semicolon, comma and newlines. */
