...
const SignatureStatus = z
  .string()
  .describe(
    "'verified-known-key' (valid, and the key is one of this account's own or contact keys) | 'valid-signature-unknown-key' (valid, but the key or certificate came only with the message) | 'bad-signature' | 'not-signed' | 'unsupported:<reason>'. A signature that verifies is still unsupported, never verified, when it is not a document signature ('unsupported:signature-type-0xNN'), its key is revoked or expired ('key-revoked', 'key-expired'), its key is a subkey the primary never bound ('subkey-not-bound') or one not allowed to sign ('key-not-for-signing'), it expired or is dated in the future ('signature-expired', 'signature-from-future'), or — S/MIME — it uses SHA-1 ('weak-hash-sha1'), its certificate was outside its validity window ('certificate-expired') or is not for e-mail ('certificate-not-for-email'), or the bytes a signature covers are not DER — a certificate ('ber-encoding') or the signed attributes ('signed-attributes-not-der'); BER wrappers (indefinite lengths, as Thunderbird/NSS send) are read. With several SignerInfos: any bad one makes the message 'bad-signature'; otherwise 'verified-known-key' if any verifies with a known certificate; otherwise the worst of the rest.",
  );
const DecryptionStatus = z.string().describe("'decrypted' | 'no-key' | 'not-encrypted' | 'failed:<reason>'. A part whose MIME type says it is encrypted (S/MIME enveloped-data) but that will not parse is 'failed:<reason>', never 'not-encrypted'.");
