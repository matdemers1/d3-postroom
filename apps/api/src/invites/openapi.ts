...
export const INVITES_ROUTES: RouteSpec[] = [
  ...
  },
  {
    ...
    tag: 'Invites',
    summary: 'Mark a CANCEL’s event STATUS:CANCELLED in the default calendar — only for a CANCEL from the event’s organizer of record, authenticated (DMARC, or DKIM) for the organizer’s domain, with a SEQUENCE not older than the stored one.',
    params: C.IdParams,
    headers: CSRF,
    responses: { '200': { description: 'Marked (or nothing to mark).', schema: 'InviteRemoveResult' }, ...COMMON, '403': err('Missing CSRF header; the CANCEL’s ORGANIZER is not the stored event’s; or it is not authenticated for the organizer’s domain.'), '409': err('Not a CANCEL, older than the stored event, or the stored event is unreadable.') },
  },
];
