// Socket plumbing for the IMAP session: a pull-based reader that can be detached (STARTTLS swaps the
// stream underneath it), an adapter that lets TLS run over a socket whose first bytes were already
// read (the PROXY v2 header), and the TLS handshake itself.
